...
public class S_Jauge_Tmp : MonoBehaviour
{
    ...
    public float _points = 0f; // Syst�me de points en float
    public float maxPoints = 100f; // Points maximaux
    public float pointsToGrow = 10f; // Points n�cessaires pour atteindre la croissance maximale

    private float _targetGrowValue; // Valeur de croissance cibl�e
    ...
    private void Update()
    {
        ...
        {
            _points = Mathf.Clamp(EnergyStore.currentEnergy, 0f, maxPoints); // Met � jour les points avec le syst�me d'�nergie
            UpdateTargetGrowValue();
            pointsToGrow = EnergyStore.energyLevels[1].requiredEnergy;
        }
        if (EnergyStore.currentLevelIndex == 1)
        {
            _points = Mathf.Clamp(EnergyStore.currentEnergy, 0f, maxPoints) - EnergyStore.energyLevels[1].requiredEnergy; // Met � jour les points avec le syst�me d'�nergie
            UpdateTargetGrowValue();
        ...
        {
            _points = Mathf.Clamp(EnergyStore.currentEnergy, 0f, maxPoints) - EnergyStore.energyLevels[2].requiredEnergy; // Met � jour les points avec le syst�me d'�nergie
            UpdateTargetGrowValue();
        ...
        {
            _points = Mathf.Clamp(EnergyStore.currentEnergy, 0f, maxPoints) - EnergyStore.energyLevels[3].requiredEnergy; // Met � jour les points avec le syst�me d'�nergie
            UpdateTargetGrowValue();
            pointsToGrow = 1200f;
        }

        // Mise � jour continue de la croissance vers la cible
